...
labs(title="Where did NBA draftees their basketball education in 2022?", 
     x="Organization",
     y="Number of Players")
...
#Gives players drafted first overall since 2000
ggplot(subset(draft_history, overall_pick %in% "1"), 
       aes(season, team_name, label = player_name)) + 
  geom_point(size = 1) +
  xlim(2000, 2022)+ 
  geom_text_repel(aes(label= player_name), size = 3, hjust = -.05) +
  labs(title="1st Overall Picks since 2000", 
       x="Season",
       y="Team Drafted to")
...
#This gives the 1st round draft pick position of every team since 2012
ggplot(subset(draft_history, season > 2012), aes(team_name, overall_pick)) +
  geom_violin() +
  ...
  ylim(1,30) +
  scale_x_discrete(guide=guide_axis(angle = 90)) +
  labs(title="Violin + Dot plot of NBA Team's 1st Round Draft Pick Selections", 
       subtitle="Average 1st Round Pick Position of Each Team",
       x="NBA Team",
       y="1st Round Draft Pick Position")

#1st round draft history of the Nets since 2012
ggplot(subset(draft_history, team_name %in% "Nets"), 
  ...
  ylim(1,30) +
  geom_text_repel(aes(label= player_name), size = 3, hjust = -.05) +
  labs(title="Draft history of Nets since 2012", 
       x="Season",
       y="Draft Position")
